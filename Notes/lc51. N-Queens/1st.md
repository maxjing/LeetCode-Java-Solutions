...
use backtracking: place, validate, undo

valide: 

1. check abs col difference = 0, if 0 means they are at same col

   2. check if abs col difference =  abs row difference

      row 0 and row 1, row diff is 1, col diff is 2 so they are ok
...
public void backTracking(int row, int n, List<Integer> subList, List<List<String>> list) {
	if(row == n) {
		list.add(generateBoard(subList, n));
		return;
	}
...
public List<String> generateBoard(List<Integer> list, int n) {
	List<String> board = new ArrayList<>();
	int size = list.size();
